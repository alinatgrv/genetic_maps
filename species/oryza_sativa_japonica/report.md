...
## О происхождении генетических координат (cM)

Важно понимать природу источника. Карта Yu et al. 2011 построена на RIL-популяции
**Zhenshan 97 × Minghui 63** — оба родителя относятся к подвиду **indica** (их гибрид —
знаменитый Shanyou 63). То есть генетические расстояния `cM` отражают рекомбинацию в
indica-кроссе. Физические координаты bin-интервалов при этом были привязаны к сборке
Nipponbare (japonica, TIGR6.1), так как это был единственный качественный референс на
момент публикации.

Поэтому данная japonica-карта = **физические позиции на japonica-геноме (IRGSP-1.0) +
генетические расстояния из indica-кросса**. Это корректная и стандартная практика:
bin-интервалы по построению упорядочены и монотонны вдоль Nipponbare, что подтверждается
коллинеарностью Spearman = 1.000 по всем 12 хромосомам (монотонность здесь именно
наблюдается, а не навязана фильтром — перенос Nipponbare→Nipponbare почти безошибочный).
Но сам факт indica-происхождения `cM` следует учитывать при интерпретации.

Та же карта Yu 2011, перенесённая на родительский indica-геном ZS97RS3, лежит в
`species/oryza_sativa_indica/` (файлы `*.zs97rs3`) — см. отчёт indica.
