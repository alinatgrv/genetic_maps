...
Для финальной карты подсолнечника была дополнительно проведена проверка коллинеарности между физической координатой маркера на референсном геноме (`pos`) и генетической координатой (`cM`).

Для каждой хромосомы была рассчитана ранговая корреляция Спирмена между `pos` и `cM`.

Файл с результатами QC:

~~~text
species/helianthus_annuus/results/qc/sunflower_final_map_collinearity_qc.tsv
~~~

Результаты показали сильную положительную корреляцию для всех 17 хромосом:
...
Все хромосомы имеют положительную ориентацию (`increasing`), то есть генетическая координата cM в целом увеличивается вместе с физической координатой pos.

Это подтверждает, что финальная карта хорошо согласована с физическим порядком маркеров на референсных хромосомах.

Отдельная строгая фильтрация по монотонности для подсолнечника не применялась. В отличие от капусты, финальная карта подсолнечника очень плотная и содержит 21,442 маркера. При такой плотности возможны локальные перестановки маркеров и большое число одинаковых или близких cM-значений. Поэтому дополнительная монотонная фильтрация могла бы удалить много маркеров без явной необходимости. Вместо этого коллинеарность была использована как QC-проверка качества итоговой карты.

Вывод: финальная карта подсолнечника проходит проверку коллинеарности. Все 17 хромосом показывают сильную положительную связь между физическими координатами и генетическими позициями.
