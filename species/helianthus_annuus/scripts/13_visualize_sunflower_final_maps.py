...
MAPS = [
    {
        "name": "bwa_exact_unique_21442",
        "label": "BWA exact unique, 21,442 markers",
        "path": SPECIES_DIR / "results/final/sunflower_genetic_map.bwa_exact_unique.tsv",
    },
]
...
def make_chromosome_summary(all_data):
    ...
    for map_info in MAPS:
        ...
        for chrom in sorted(by_chr, key=chr_sort_key):
            ...
            physical_span_bp = pos_max - pos_min
            genetic_span_cm = cm_max - cm_min
            ...
            rows.append({
                ...
                "markers_per_Mbp": len(chrom_rows) / (physical_span_bp / 1_000_000) if physical_span_bp > 0 else "",
                "markers_per_cM": len(chrom_rows) / genetic_span_cm if genetic_span_cm > 0 else "",
            })

    out_tsv = QC_DIR / "sunflower_final_map_chromosome_summary.tsv"
...
def make_density_10cm(all_data):
    ...
    out_tsv = QC_DIR / "sunflower_marker_density_10cM_bins.tsv"
    with open(out_tsv, "w", encoding="utf-8", newline="") as handle:
    ...
    make_density_svg(density_rows, FIG_DIR / "sunflower_marker_density_10cM.svg")
...
def make_density_svg(density_rows, out_path):
    ...
    body = []
    body.append(f'<text x="{width/2}" y="38" text-anchor="middle" font-family="Arial" font-size="26" font-weight="bold">Sunflower final genetic map: marker density in 10 cM bins</text>\n')
    ...
    for panel_i, map_info in enumerate(MAPS):
        ...
        for j, bin_start in enumerate(bins):
            ...
            body.append(f'<text x="{left_margin - 15}" y="{y + 19}" text-anchor="end" font-family="Arial" font-size="12">{bin_start}-{bin_start+bin_size}</text>\n')
            ...
            for i, chrom in enumerate(chroms):
                ...
                if count > 0:
                    ...
                    body.append(f'<text x="{x + (cell_w-4)/2}" y="{y + 19}" text-anchor="middle" font-family="Arial" font-size="12" fill="{text_color}">{count}</text>\n')
...
def main():
    ...
    make_rug_map_svg(
        ...
        out_path=FIG_DIR / "sunflower_genetic_map_coverage.svg",
        title="Sunflower final genetic map: marker coverage by cM"
    )
    ...
    make_rug_map_svg(
        ...
        out_path=FIG_DIR / "sunflower_physical_map_coverage.svg",
        title="Sunflower final genetic map: marker coverage by physical position"
    )
    ...
    print("Done.")
    print("Figures:")
    print(FIG_DIR / "sunflower_genetic_map_coverage.svg")
    print(FIG_DIR / "sunflower_physical_map_coverage.svg")
    print(FIG_DIR / "sunflower_marker_density_10cM.svg")
    print("Tables:")
    print(summary_tsv)
    print(density_tsv)
