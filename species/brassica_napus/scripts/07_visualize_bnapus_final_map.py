...
# Визуализация финальной генетической карты рапса:
...
#
# Плюс сводные таблицы по хромосомам и по плотности. SVG генерируем без внешних
# зависимостей (как в остальных культурах проекта).
#
...
MAPS = [
    {
        "name": "strict_322",
        "label": "DArT consensus, strict unique map (322 markers)",
        "path": SPECIES_DIR / "results/final/brassica_napus_genetic_map.tsv",
    },
]
...
def make_chromosome_summary(all_data):
    ...
    for map_info in MAPS:
        ...
        for chrom in sorted(by_chr, key=chr_sort_key):
            ...
            rows.append({
                ...
                "chr": chrom,
                "n_markers": len(chrom_rows),
                "pos_min": int(min(pos_values)),
                "pos_max": int(max(pos_values)),
    ...
    out_tsv = QC_DIR / "brassica_napus_final_map_chromosome_summary.tsv"
...
def make_rug_map_svg(all_data, value_col, unit_label, out_path, title):
    ...
    if value_col == "pos":
        ...
        global_max = math.ceil(max(all_values) / 10) * 10
    ...
    for panel_i, map_info in enumerate(MAPS):
        ...
        for i, chrom in enumerate(chroms):
            ...
            if vals:
                y1 = y_scale(min(vals), panel_top)
                y2 = y_scale(max(vals), panel_top)
            else:
            ...
            body.append(f'<text x="{x}" y="{plot_bottom + 28}" text-anchor="middle" font-family="Arial" font-size="14">{svg_escape(chrom)}</text>\n')
...
def make_density_10cm(all_data):
    ...
    for map_info in MAPS:
        ...
        for chrom, values in sorted(by_chr.items(), key=lambda x: chr_sort_key(x[0])):
            ...
            while current < cmax:
                ...
                current += bin_size
    out_tsv = QC_DIR / "brassica_napus_marker_density_10cM_bins.tsv"
    with open(out_tsv, "w", encoding="utf-8", newline="") as handle:
    ...
    make_density_svg(density_rows, FIG_DIR / "brassica_napus_marker_density_10cM.svg")
    return out_tsv
...
def make_density_svg(density_rows, out_path):
    ...
    body = []
    body.append(f'<text x="{width/2}" y="38" text-anchor="middle" font-family="Arial" font-size="26" font-weight="bold">Rapeseed (Brassica napus) final genetic map: marker density in 10 cM bins</text>\n')
    ...
    for panel_i, map_info in enumerate(MAPS):
        ...
        for i, chrom in enumerate(chroms):
            ...
            body.append(f'<text x="{x + cell_w/2}" y="{panel_top + 30}" text-anchor="middle" font-family="Arial" font-size="13">{svg_escape(chrom)}</text>\n')
...
def main():
    ...
    make_rug_map_svg(all_data, "cM", "Genetic position, cM",
                     FIG_DIR / "brassica_napus_genetic_map_coverage.svg",
                     "Rapeseed (Brassica napus) final genetic map: marker coverage by cM")
    make_rug_map_svg(all_data, "pos", "Physical position, Mb",
                     FIG_DIR / "brassica_napus_physical_map_coverage.svg",
                     "Rapeseed (Brassica napus) final genetic map: marker coverage by physical position")
    density_tsv = make_density_10cm(all_data)

    print("Done.")
    print("Figures:")
    print(FIG_DIR / "brassica_napus_genetic_map_coverage.svg")
    print(FIG_DIR / "brassica_napus_physical_map_coverage.svg")
    print(FIG_DIR / "brassica_napus_marker_density_10cM.svg")
    print("Tables:")
    print(summary_tsv)
    print(density_tsv)
